...
They need to be in
  [JSON Schema](http://json-schema.org/) format, check [here](/docs/v0.28.x/Validation-And-Serialize) for more info.
...
* `beforeHandler(request, reply, done)`: a [function](/docs/v0.28.x/Hooks#before-handler) called just before the request handler, useful if you need to perform authentication at route level for example, it could also be and array of functions.
* `handler(request, reply)`: the function that will handle this request.
* `schemaCompiler(schema)`: the function that build the schema for the validations. See [here](/docs/v0.28.x/Validation-And-Serialize#schema-compiler)

  `request` is defined in [Request](/docs/v0.28.x/Request).

  `reply` is defined in [Reply](/docs/v0.28.x/Reply).
