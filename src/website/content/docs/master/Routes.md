...
path: /docs/master/Routes
version: master

github_url: https://github.com/fastify/fastify/blob/master/docs/Routes.md
---
...
They need to be in
  [JSON Schema](http://json-schema.org/) format, check [here](/docs/master/Validation-and-Serialization) for more info.
...
* `beforeHandler(request, reply, done)`: a [function](/docs/master/Hooks#before-handler) called just before the request handler, useful if you need to perform authentication at route level for example, it could also be and array of functions.
* `handler(request, reply)`: the function that will handle this request.
* `schemaCompiler(schema)`: the function that build the schema for the validations. See [here](/docs/master/Validation-and-Serialization#schema-compiler)

  `request` is defined in [Request](/docs/master/Request).

  `reply` is defined in [Reply](/docs/master/Reply).
