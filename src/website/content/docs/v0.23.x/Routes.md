...
They need to be in
  [JSON Schema](http://json-schema.org/) format, check [here](/docs/v0.23.x/Validation-And-Serialize) for more info.
...
* `handler(request, reply)`: the function that will handle this request.
  ...
  `request` is defined in [Request](/docs/v0.23.x/Request).

  `reply` is defined in [Reply](/docs/v0.23.x/Reply).
