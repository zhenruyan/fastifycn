...
They need to be in
  [JSON Schema](http://json-schema.org/) format, check [here](/docs/v0.25.x/Validation-And-Serialize) for more info.
...
* `beforeHandler(request, reply, done)`: a [function](/docs/v0.25.x/Hooks#before-handler) called just before the request handler, useful if you need to perform authentication at route level for example, it could also be and array of functions.
* `handler(request, reply)`: the function that will handle this request.

  `request` is defined in [Request](/docs/v0.25.x/Request).

  `reply` is defined in [Reply](/docs/v0.25.x/Reply).
