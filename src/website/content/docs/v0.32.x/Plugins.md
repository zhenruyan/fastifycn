...
A plugin can be a set of routes, a server [decorator](/docs/v0.32.x/Decorators) or whatever. The API that you will need to use one or more plugins, is `register`.  
...
You already see in the [getting started](/docs/v0.32.x/Getting-Started#register) section how using this API is pretty straightforward.
```
...
If you pass an option with the key `prefix` with a `string` value, Fastify will use it to prefix all the routes inside the register, for more info check [here](/docs/v0.32.x/Routes#route-prefixing).  
...
```
Sometimes, you will need to know when the server is about to close, for example because you must close a connection to a database. To know when this is going to happen, you can use the [`'onClose'`](/docs/v0.32.x/Hooks#on-close) hook.
...
### Handle the scope
If you are using `register` only for extending the functionality of the server with  [`decorate`](/docs/v0.32.x/Decorators), it is your responsibility to tell Fastify to not create a new scope, otherwise your changes will not be accessible by the user in the upper scope.
