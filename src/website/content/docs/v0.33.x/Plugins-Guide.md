...
path: /docs/v0.33.x/Plugins-Guide
version: v0.33.x
...
Your routes, your utilities and so on are all plugins. And to add a new plugin, whatever its functionality is, in Fastify you have a nice and unique api to use: [`register`](/docs/v0.33.x/Plugins).
```js
fastify.register(
  require('./my-plugin'),
  { options },
...
Create a decorator is extremely easy, just use the [`decorate`](/docs/v0.33.x/Decorators) api:
```js
...
We've seen how extend server functionalities and how handle the encapsulation system, but what if you need to add a function that must be executed every time that the server "[emits](/docs/v0.33.x/Lifecycle)" an event?
...
So what can you do to avoid this annoying issue? Yes, you are right, use an [hook](/docs/v0.33.x/Hooks)!  
```js
fastify.decorate('util', (req, key, value) => { req.key = value })
...
## Middlewares
Fastify [supports](/docs/v0.33.x/Middlewares) out of the box Express/Restify/Connect middlewares, this means that you can just drop-in your old code and it will work! *(faster, by the way)*  
